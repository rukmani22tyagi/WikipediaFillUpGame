...
@IBDesignable open class ActiveLabel: UILabel {
    ...
    var postionIntext: Int = 0  //rukmani
    ...
    open func handleCustomTap(for type: ActiveType, handler: @escaping (String, Int) -> ()) {
    ...
    internal var customTapHandlers: [ActiveType : ((String, Int) -> ())] = [:]
    ...
    fileprivate func element(at location: CGPoint) -> ElementTuple? {
        ...
        for element in activeElements.map({ $0.1 }).joined() {
            if index >= element.range.location && index <= element.range.location + element.range.length {
                postionIntext = element.range.location
                return element
            }
        }
    ...
    fileprivate func didTap(_ element: String, for type: ActiveType) {
        ...
        elementHandler(element, self.postionIntext)
    }
}
